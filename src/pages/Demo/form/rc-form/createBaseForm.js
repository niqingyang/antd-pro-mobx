...
import {action as mobxAction, extendObservable, observable, computed, toJS} from "mobx";
...
const ITEM_PROPS = ['colon', 'extra', 'hasFeedback', 'help', 'htmlFor', 'label', 'labelCol', 'required', 'validateStatus', 'wrapperCol'];
const FIELD_PROPS = ['value', 'errors', 'name'];
const FIELD_META_PROPS = ['props', 'getValueFromEvent', 'initialValue', 'normalize', 'preserve', 'rules', 'trigger', 'validateFirst', 'validateTrigger', 'valuePropName'];
    ...
    return function decorate(WrappedComponent) {
        ...
        class Form extends React.Component {
            ...
            constructor(props) {
                ...
                if (formFields) {
                    this.formFields = formFields && formFields(props);
                } else if (fields) {
            ...
            setFormField = (name, field) => {
                ...
                const formField = get(this.formFields, name, {});

                set(this.formFields, name, {
                    ...formField,
                    ...field
                });
            }
            ...
            getFormField = (name) => {
                ...
                if (!name) {
                    throw new Error('Must call `getFormField` with valid name string!');
                }
                ...
                const props = {
                    field: pick(formField, FIELD_PROPS),
                    fieldMeta: pick(formField, FIELD_META_PROPS),
                    itemProps: pick(formField, ITEM_PROPS),
                };

                this.fieldsStore.setFlattenFields({
                    [name]: createFormField(props.field)
                });

                this.fieldsStore.setFieldMeta(name, props.fieldMeta);

                const field = this.fieldsStore.getField(name);
                const fieldMeta = this.fieldsStore.getFieldMeta(name);

                props.field = pick(field, FIELD_PROPS);

                props.fieldMeta = pick({
                    ...fieldMeta,
                    ...formField
                }, FIELD_META_PROPS);

                props.itemProps = pick(formField, ITEM_PROPS);
            ...
            get = (name) => {
                ...
                if (!this.fieldsStore.hasField(name) || !this.fieldsStore.hasFieldMeta(name)) {
